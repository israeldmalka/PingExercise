...
class Client:
    ...
    def send_one_packet(self):
        # send one packet and waits to receive the packet from the server
        
        packet_to_send = bytes(self.packet_size)

        if self.protocol == 'TCP':
            self.socket.send(packet_to_send)    
            received_packet = self.socket.recv(self.packet_size)

        if self.protocol == 'UDP':
            server_address = (self.ip_address, self.port)
            self.socket.sendto(packet_to_send, server_address)
            (received_packet, _) = self.socket.recvfrom(self.packet_size)
